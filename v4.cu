...
// Kernel to add bias after cuBLAS GEMM
__global__ void addBiasKernel(Real* C, Real* bias, int rows, int cols) {
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    if (row < rows && col < cols) {
        C[row * cols + col] += bias[row];
    }
}
...
void forward_gpu(NeuralNetwork* net, Real* d_input, Real* d_hidden, Real* d_output, cudaStream_t stream, Config* config) {
    // cuBLAS matrix multiplication: d_hidden = W1 * d_input
    float alpha = 1.0f, beta = 0.0f;
    CUBLAS_CHECK(cublasSetStream(net->cublas_handle, stream));
    CUBLAS_CHECK(cublasGemmEx(net->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N,
                              HIDDEN_SIZE, 1, INPUT_SIZE,
                              &alpha, net->d_W1, CUDA_R_32F, HIDDEN_SIZE,
                              d_input, CUDA_R_32F, INPUT_SIZE,
                              &beta, d_hidden, CUDA_R_32F, HIDDEN_SIZE,
                              CUBLAS_COMPUTE_32F_FAST_TF32, CUBLAS_GEMM_DEFAULT_TENSOR_OP));

    // Add bias
    dim3 blockDim, gridDim;
    computeGridBlockDim(HIDDEN_SIZE, 1, &gridDim, &blockDim, config);
    addBiasKernel<<<gridDim, blockDim, 0, stream>>>(d_hidden, net->d_b1, HIDDEN_SIZE, 1);
    ...
    // cuBLAS matrix multiplication: d_output = W2 * d_hidden
    CUBLAS_CHECK(cublasGemmEx(net->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N,
                              OUTPUT_SIZE, 1, HIDDEN_SIZE,
                              &alpha, net->d_W2, CUDA_R_32F, OUTPUT_SIZE,
                              d_hidden, CUDA_R_32F, HIDDEN_SIZE,
                              &beta, d_output, CUDA_R_32F, OUTPUT_SIZE,
                              CUBLAS_COMPUTE_32F_FAST_TF32, CUBLAS_GEMM_DEFAULT_TENSOR_OP));

    // Add bias
    computeGridBlockDim(OUTPUT_SIZE, 1, &gridDim, &blockDim, config);
    addBiasKernel<<<gridDim, blockDim, 0, stream>>>(d_output, net->d_b2, OUTPUT_SIZE, 1);
...
void backward_gpu(NeuralNetwork* net, Real* d_input, Real* d_hidden, Real* d_output, Real* d_target, Real* d_d_output, Real* d_d_hidden, cudaStream_t stream, Config* config) {
    ...
    // Update W2 using cuBLAS: W2 -= lr * d_output * hidden^T
    float alpha = -LEARNING_RATE, beta = 1.0f;
    CUBLAS_CHECK(cublasSetStream(net->cublas_handle, stream));
    CUBLAS_CHECK(cublasGemmEx(net->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T,
                              OUTPUT_SIZE, HIDDEN_SIZE, 1,
                              &alpha, d_d_output, CUDA_R_32F, OUTPUT_SIZE,
                              d_hidden, CUDA_R_32F, HIDDEN_SIZE,
                              &beta, net->d_W2, CUDA_R_32F, OUTPUT_SIZE,
                              CUBLAS_COMPUTE_32F_FAST_TF32, CUBLAS_GEMM_DEFAULT_TENSOR_OP));

    // Update W1 using cuBLAS: W1 -= lr * d_hidden * input^T
    CUBLAS_CHECK(cublasGemmEx(net->cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T,
                              HIDDEN_SIZE, INPUT_SIZE, 1,
                              &alpha, d_d_hidden, CUDA_R_32F, HIDDEN_SIZE,
                              d_input, CUDA_R_32F, INPUT_SIZE,
                              &beta, net->d_W1, CUDA_R_32F, HIDDEN_SIZE,
                              CUBLAS_COMPUTE_32F_FAST_TF32, CUBLAS_GEMM_DEFAULT_TENSOR_OP));
...
int main() {
    printf("MNIST Neural Network - Version 4: Tensor Cores with cuBLAS and TF32\n\n");

    // Print applied optimizations
    printf("=== Applied Optimizations ===\n");
    printf("1. Tensor Core Acceleration: Using cuBLAS with TF32 precision for matrix multiplications to leverage Tensor Cores on Ampere+ GPUs.\n");
    printf("2. Dynamic Launch Configuration: Block and grid dimensions are computed dynamically using configurable block sizes (1D: %d threads, 2D: %dx%d) to optimize kernel launches.\n",
           config.block_size_1d, config.block_size_2d_x, config.block_size_2d_y);
    printf("3. Variable Precision: Supports TF32 via cuBLAS for high performance while maintaining FP32-like accuracy.\n");
    printf("4. Parallel Softmax: Softmax kernel uses parallel reductions for max and sum operations, distributing computation across threads.\n");
    printf("5. Pinned Host Memory: Used cudaMallocHost for host-side allocations (temp_W1, temp_W2) to enable faster device-to-host transfers.\n");
    printf("6. Reused Allocated Memory: Allocated d_hidden, d_output, d_d_output, and d_d_hidden once at the start, reusing them across iterations and epochs.\n");
    printf("7. Asynchronous Transfers with Variable Streams: Used %d CUDA streams to overlap data transfers with computation.\n", config.num_streams);
    printf("\n");
...
// nvcc -arch=sm_80 -O2 -lcublas -o n v4.cu && ./n