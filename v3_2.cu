...
void train(NeuralNetwork* net, double** images, double** labels, int numImages, bool use_gpu, double* total_time, double* loss_out, double* train_acc_out) {
    ...
    if (use_gpu) {
        ...
        // Allocate pinned host and device memory for two batches (for streams)
        double *h_batch_images[2], *h_batch_labels[2], *h_batch_outputs[2];
        double *d_batch_images[2], *d_batch_labels[2], *d_batch_outputs[2];
        for (int s = 0; s < 2; s++) {
            CUDA_CHECK(cudaMallocHost(&h_batch_images[s], BATCH_SIZE * INPUT_SIZE * sizeof(double)));
            CUDA_CHECK(cudaMallocHost(&h_batch_labels[s], BATCH_SIZE * OUTPUT_SIZE * sizeof(double)));
        ...
        for (int epoch = 0; epoch < EPOCHS; epoch++) {
            ...
            for (int i = 0; i < numImages; i += BATCH_SIZE) {
                ...
                for (int b = 0; b < batch_size; b++) {
                    ...
                    for (int j = 0; j < OUTPUT_SIZE; j++) {
                        h_batch_labels[stream_idx][b * OUTPUT_SIZE + j] = labels[img_idx][j];
                    }
                }

                // Asynchronously transfer batch data to device
                ...
                CUDA_CHECK(cudaMemcpyAsync(d_batch_labels[stream_idx], h_batch_labels[stream_idx],
                                           batch_size * OUTPUT_SIZE * sizeof(double), cudaMemcpyHostToDevice, stream));
                ...
                // Synchronize stream to compute loss and accuracy
        ...
        *total_time = get_gpu_time(start, stop);
        ...
        for (int s = 0; s < 2; s++) {
            ...
            CUDA_CHECK(cudaFreeHost(h_batch_labels[s]));
...
void evaluate(NeuralNetwork* net, double** images, double** labels, int numImages, bool use_gpu, double* test_acc_out) {
    ...
    if (use_gpu) {
        ...
        // Allocate pinned host and device memory for two batches
        double *h_batch_images[2], *h_batch_outputs[2];
        double *d_batch_images[2], *d_batch_outputs[2];
        for (int s = 0; s < 2; s++) {
            CUDA_CHECK(cudaMallocHost(&h_batch_images[s], BATCH_SIZE * INPUT_SIZE * sizeof(double)));
            CUDA_CHECK(cudaMallocHost(&h_batch_outputs[s], BATCH_SIZE * OUTPUT_SIZE * sizeof(double)));
            CUDA_CHECK(cudaMalloc(&d_batch_images[s], BATCH_SIZE * INPUT_SIZE * sizeof(double)));
        ...
        for (int i = 0; i < numImages; i += BATCH_SIZE) {
            ...
            // Synchronize stream to compute accuracy
            CUDA_CHECK(cudaStreamSynchronize(stream));
...
int main() {
    ...
    printf("3. Asynchronous Transfers with Streams: Used two CUDA streams to overlap data transfers (batch images, labels, and outputs) with computation, processing one batch while transferring another.\n");
